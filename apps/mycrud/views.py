...
from apps.mycrud.forms import UserForm
...
@mycrud.route("/users/new", methods=["GET", "POST"])
def create_user():
    # UserForm을 인스턴스화한다
    form = UserForm()
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("mycrud.users"))
    return render_template("mycrud/create.html", form=form)
...
def users():
    ...
    print(users)
    return render_template('mycrud/index.html',users=users)

@mycrud.route('/user/<user_id>',methods=["POST","GET"])
def edit_user(user_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("mycrud.users"))
    return render_template('mycrud/edit.html',user=user, form=form)
